...
namespace Zapotlan.PortalWeb.Admin.Infrastructure.Validators
{
    ...
    public class AreaPutValidator : AbstractValidator<AreaEditDto>
    {
        public AreaPutValidator()
        {
            ...
            RuleFor(i => i.Clave)
                .MaximumLength(20).WithMessage("La {PropertyName} no puede ser mayor a {MaxLength} carácteres");
            ...
            RuleFor(i => i.NombreCorto)
                .MaximumLength(50).WithMessage("El {PropertyName} no puede ser mayor a {MaxLength} carácteres");

            RuleFor(i => i.Descripcion)
                .MaximumLength(255).WithMessage("El {PropertyName} no puede ser mayor a {MaxLength} carácteres");

            RuleFor(i => i.Tipo)
                .IsInEnum().WithMessage("El valor {PropertyValue} para {PropertyName} no es un valor válido");

            RuleFor(i => i.UbicacionDescripcion)
                .MaximumLength(500).WithMessage("La {PropertyName} no puede ser mayor a {MaxLength} carácteres");

            RuleFor(i => i.Tags)
                .MaximumLength(400).WithMessage("Los {PropertyName} no puede ser mayor a {MaxLength} carácteres");
